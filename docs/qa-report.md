...
- Rendered-route tests: 14 pass, covering the homepage, the Work archive, three Work files, Notes, About, Résumé, Speaking, Press, Library, default-deny indexing, installable icons and the bespoke 404.
- Diff check: pass.

## Remote evidence

Phase 3 was pushed to `origin/atelier-v1` and published through immutable Cloudflare Worker versions. Each acceptance run first matched the full `X-Deploy-SHA` response header to the pushed Git revision and passed Major remote-web preflight.

The remote matrix covered:

- all public routes at 1440, 1024, 768 and 390 pixel widths with one H1, successful responses and no horizontal overflow;
...
- Speaking propositions, Press biographies, copy feedback, the 2.6 MB ZIP and all three individual portrait downloads;
- route-specific metadata, social images, default-deny `robots.txt`, an empty preview sitemap, the manifest, platform icons and a real useful 404;
- mobile layout-shift tracing and the sound-offer positioning that removed the measured first-view shift;
