...
export function PressPage() {
  return (
    <main className={styles.page} id="main-content">
      ...
      <section className={styles.assets} aria-labelledby="assets-title"><header><p className={styles.sectionLabel}>02 / Portraits</p><div><h2 id="assets-title">Three useful frames.</h2><p>High-resolution JPGs for editorial and event-programme use.</p></div></header><div className={styles.assetGrid}>{pressPortraits.map((portrait) => <article key={portrait.label}><Image src={portrait.display} width={portrait.width} height={portrait.height} sizes="(max-width: 900px) 100vw, 33vw" alt={portrait.alt} /><div><h3>{portrait.label}</h3><p>{portrait.width} × {portrait.height} · JPG</p><ResourceAction href={portrait.download} download indicator="download" sound="press">Download {portrait.label.toLowerCase()}</ResourceAction></div></article>)}</div><p className={styles.useLine}>Identify the subject as Chuka Dele-Oyeleru. Do not use generative editing to change his likeness.</p></section>
